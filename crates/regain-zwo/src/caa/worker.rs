...
pub fn run(mut args: Vec<String>) -> Result<()> {
    ...
    let Some(command) = args.first().map(String::as_str) else {
        bail!(
            "usage: regain-device zwo caa list|list-details|status|serve|exercise [--path HID-PATH | --serial SERIAL]"
        );
    };
    if command == "--help" {
        println!(
            "regain-device zwo caa list|list-details|status|serve|exercise [--path HID-PATH | --serial SERIAL]\nNo SDK required. Exercise moves at most eight degrees from the initial position and restores settings.\nServe accepts one JSON command per line; EOF cancels queued moves and stops the motor."
        );
        return Ok(());
    }
    ensure!(
    ...
    if simulate {
        let mut caa = Caa::connect(crate::caa::simulation::Sim::default())?;
...
fn serve<T: crate::caa::Transport>(caa: Caa<T>) -> Result<()> {
    let mut controller = Controller::new(caa)?;
    regain_worker::serve(
        &mut controller,
        Duration::from_millis(100),
        |controller, request| controller.request(&request),
        Controller::tick,
    );
