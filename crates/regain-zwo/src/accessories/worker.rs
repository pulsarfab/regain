use crate::accessories::{Accessory, Calibration, Kind, Status, Transport, simulation::Sim};
...
fn discover(kind: Kind, simulate: bool) -> Result<Vec<Device>> {
    ...
    for info in crate::hid::enumerate(0x03c3, kind.product_id())? {
        match crate::hid::Device::open(&info, 0x03c3, kind.product_id())
...
pub fn run(args: Vec<String>) -> Result<()> {
    let kind = match args.first().map(String::as_str) {
        ...
        Some("eaf") => Kind::Eaf,
        _ => bail!(
            "usage: regain-device zwo efw|eaf list-details|status|serve [--serial SERIAL] [--simulate]"
        ),
    };
    ...
    let rx = regain_worker::stdin_requests();
        ...
        match rx.recv_timeout(Duration::from_millis(100)) {
            Ok(line) => {
                let response = (|| -> Result<Value> {
                    ...
                    let request: Value = regain_worker::parse(&line)?;
                ...
                if regain_worker::write_reply(response).is_err() {
                    break;
                }
            }
