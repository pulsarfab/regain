...
use crate::asi::sdk::{Exposure, Host};
...
const HELP: &str = "PulsarFab regain SDK camera commands:
  regain-device zwo camera-sdk --list [--sdk /absolute/path/to/library]
  regain-device zwo camera-sdk --inspect [--camera NAME] [--serial HEX]
  regain-device zwo camera-sdk --capture --output NEW_DIRECTORY [--camera NAME] [--serial HEX]
