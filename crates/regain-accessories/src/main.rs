...
use regain_accessories::{Accessory, Calibration, Kind, Status, Transport, simulation::Sim};
...
fn discover(kind: Kind, simulate: bool) -> Result<Vec<Device>> {
    ...
    for info in regain_hid::enumerate(0x03c3, kind.product_id())? {
        match regain_hid::Device::open(&info, 0x03c3, kind.product_id())
...
fn main() -> Result<()> {
    ...
    let kind = match args.first().map(String::as_str) {
        ...
        Some("eaf") => Kind::Eaf,
        _ => bail!(
            "usage: regain-accessories efw|eaf list-details|status|serve [--serial SERIAL] [--simulate]"
        ),
    };
