...
impl Accessory {
    ...
    pub async fn request(&self, member: &str, put: bool, p: &Params) -> Result<Value> {
            ...
            match member {
                ...
                "supportedactions" => {
                    return Ok(if self.kind == "efw" {
                        json!(["ZwoGain.Status", "ZwoGain.Identity", "ZwoGain.Calibrate"])
                    } else {
                        json!(["ZwoGain.Status", "ZwoGain.Identity"])
                    });
                }
                _ => (),
            }
        ...
        if member == "action" && put {
            let command = match p.string("Action")?.to_ascii_lowercase().as_str() {
                "zwogain.status" => "status",
                "zwogain.identity" => "identity",
                "zwogain.calibrate" if self.kind == "efw" => "calibrate",
