...
impl Device {
    ...
    pub fn put(self: &Arc<Self>, member: &str, p: &Params, client: u32) -> Result<Value> {
        ...
        if member == "action" {
            let name = p.string("Action")?.to_lowercase();
            let args = p.string("Parameters")?;
            return match name.as_str() {
                "zwogain.diagnostics" => Ok(json!(serde_json::to_string(&status)?)),
                "zwogain.controls" => Ok(json!(serde_json::to_string(
                    &status
                        .controls
                        .values()
                        .map(|c| {
