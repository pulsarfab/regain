...
if (args.Length != 2) throw new ArgumentException("Usage: ZwoGain.Packaging <archive.zip> <manifest.json>");
var archivePath = Path.GetFullPath(args[0]);
var plugin = new ZwoGainPlugin();
string version = typeof(ZwoGainPlugin).Assembly.GetName().Version!.ToString();
string archiveName = $"ZwoGain-{version}.zip";
...
{
    string[] required = ["ZwoGain.NINA.dll", "ZwoGain.Core.dll", "ZwoGain.Rotator.dll", "zwogain-caa.exe", "zwogain-accessories.exe", "zwogain-ofp2.exe", "zwogain-fc3.exe", "caa.md", "zwogain-host.exe", "zwogain-direct.exe", "zwogain-camera.exe", "zwogain-alpaca.exe", "ASICamera2.dll",
...
var manifest = new PluginManifest
{
    ...
    Descriptions = new PluginDescription
    {
        ...
        FeaturedImageURL = $"{baseUrl}/zwogain.png", ScreenshotURL = "", AltScreenshotURL = ""
    },
