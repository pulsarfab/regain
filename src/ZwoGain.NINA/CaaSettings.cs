...
{
    public static void Show(CaaSession session, bool ownedByNina)
    {
        var dispatcher = Application.Current?.Dispatcher;
        if (dispatcher is not null && !dispatcher.CheckAccess()) {
            dispatcher.Invoke(() => Show(session, ownedByNina)); return;
        }
        ...
        var heading = new TextBlock { Text = "Choose a rotator. Changes apply immediately.", Margin = new Thickness(0, 0, 0, 12) };
        ...
        static double Value(TextBox box, double min, double max, bool integer = false) {
            if (!double.TryParse(box.Text, NumberStyles.Float, CultureInfo.InvariantCulture, out double value) ||
                !double.IsFinite(value) || value < min || value > max || (integer && value != Math.Truncate(value)))
        ...
        var device = Tab("Device", "Select the CAA to use with NINA.");
        ...
        var deviceActions = new StackPanel { Orientation = Orientation.Horizontal }; device.Children.Add(deviceActions);
        async Task Discover() {
            ...
            picker.SelectedItem = choices.FirstOrDefault(c => c.Serial == saved?.Serial) ?? choices.FirstOrDefault();
        ...
        Button(deviceActions, "Save selection", () => { SaveSelection(); feedback.Text = "Selection saved."; return Task.CompletedTask; }, offlineControls);
        ...
        if (ownedByNina) device.Children.Add(new TextBlock { Text = "Connected through NINA. Use the equipment pane to disconnect.", TextWrapping = TextWrapping.Wrap });
        else {
            Button(device, "Connect", async () => { SaveSelection(); await Task.Run(session.Connect); await ReadDetails(); }, offlineControls);
            Button(device, "Disconnect", async () => { await Task.Run(session.Disconnect); moving = false; live.Text = "Disconnected"; }, connectedControls);
        }
        Button(device, "Read device", ReadDetails, connectedControls);
