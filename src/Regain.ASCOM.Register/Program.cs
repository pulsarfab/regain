            ...
            foreach (var view in new[] { RegistryView.Registry32, RegistryView.Registry64 }) {
                ...
                if (remove) for (int slot = 1; slot <= 4; slot++) {
                    ...
                    if (existing is not null && (!Uri.TryCreate(existing.GetValue("CodeBase") as string, UriKind.Absolute, out var installed) ||
                        ...
                        throw new InvalidOperationException("Unregister from the currently installed PulsarFab regain directory.");
                }
                if (remove) {
                ...
                for (int slot = 1; slot <= 4; slot++) {
                    ...
                    else { using var chooser = root.CreateSubKey(key); chooser.SetValue(null, "PulsarFab regain Retryable Camera " + slot); }
                }
                string rotatorKey = @"Software\ASCOM\Rotator Drivers\ASCOM.ZWOgain.Rotator";
                if (remove) root.DeleteSubKeyTree(rotatorKey, false);
                else { using var chooser = root.CreateSubKey(rotatorKey); chooser.SetValue(null, "PulsarFab regain CAA Rotator"); }
                foreach (var device in new[] { (Type: "FilterWheel", Name: "PulsarFab regain EFW Filter Wheel"), (Type: "Focuser", Name: "PulsarFab regain EAF Focuser") }) {
