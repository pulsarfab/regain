...
public abstract class AccessoryBase : IDisposable
{
    ...
    private AccessorySession NewSession() => new(Environment.GetEnvironmentVariable("ZWOGAIN_ACCESSORY_WORKER") ?? Path.Combine(Path.GetDirectoryName(typeof(AccessoryBase).Assembly.Location)!, "zwogain-accessories.exe"), Kind, AccessorySession.SettingsPath(Kind, "ascom"));
    ...
    public string Action(string ActionName, string ActionParameters) => ActionName.ToLowerInvariant() switch {
        ...
        "zwogain.identity" => Read(d => d.Request(new { command = "identity" }).GetRawText()),
        "zwogain.calibrate" when Kind == "efw" => Read(d => { d.Calibrate(); return "null"; }),
...
public sealed class EfwFilterWheel : AccessoryBase, IFilterWheelV2
{
    ...
    public override string Name => "ZWOgain EFW Filter Wheel";
...
public sealed class EafFocuser : AccessoryBase, IFocuserV3
{
    ...
    public override string Name => "ZWOgain EAF Focuser";
