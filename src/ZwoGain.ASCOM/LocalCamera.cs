...
internal sealed class LocalCamera : IDisposable
{
    ...
    private void Start()
    {
        ...
        string executable = Environment.GetEnvironmentVariable("ZWOGAIN_CAMERA_WORKER") ?? Path.Combine(directory, "zwogain-camera.exe");
        string profiles = Environment.GetEnvironmentVariable("ZWOGAIN_ASCOM_PROFILES") ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ZwoGain", "ASCOM");
        string path = Path.GetFullPath(Path.Combine(profiles, $"camera-{slot + 1}.json"));
        worker = new Process { StartInfo = new ProcessStartInfo(executable, "--profiles \"" + path + "\"" + (Environment.GetEnvironmentVariable("ZWOGAIN_ASCOM_SIMULATE") == "1" ? " --simulate" : "")) {
