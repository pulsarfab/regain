...
public sealed class ResilientCamera : BaseINPC, ICamera
{
    ...
    public async Task<bool> Connect(CancellationToken token)
    {
        ...
        {
            selected = selectionStore.Load() ?? throw new InvalidOperationException("Choose a camera in the PulsarFab regain setup dialog first.");
            SelectCamera(selected.Camera);
        }
    ...
    private async Task Poll(CameraSession owner, CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            catch (Exception e) { Logger.Warning($"PulsarFab regain telemetry: {e.Message}"); }
        }
    }
    ...
    private void ExtendNinaTimeout(CameraSession owner, double seconds)
    {
        ...
        Logger.Info($"PulsarFab regain temporarily extended NINA readiness timeout from {originalTimeout} to {budget} seconds for this capture");
    }
