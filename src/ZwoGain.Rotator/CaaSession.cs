...
public sealed class CaaSession : IDisposable
{
    ...
    public static readonly string[] Actions = { "ZwoGain.CAA.Status", "ZwoGain.CAA.Settings", "ZwoGain.CAA.Identity", "ZwoGain.CAA.SetReference", "ZwoGain.CAA.SetLimit", "ZwoGain.CAA.SetBeep", "ZwoGain.CAA.SetAlias", "ZwoGain.CAA.RotateUnwrapped", "ZwoGain.CAA.ResetOrigin" };
    public string Action(string name, string parameters)
    {
        lock (gate) {
        int index = Array.FindIndex(Actions, a => a.Equals(name, StringComparison.OrdinalIgnoreCase));
