...
public sealed class AccessorySession : IDisposable
{
    ...
    public static string SettingsPath(string kind, string frontend) => Path.Combine(
        Environment.GetEnvironmentVariable("ZWOGAIN_ACCESSORY_SETTINGS") ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ZwoGain", "Accessories"), kind + "-" + frontend + ".json");
