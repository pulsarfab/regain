...
| ASI2600MM Pro main sensor (SDK name includes Duo) | Direct RAW16 capture, bins 1–4, calibration/defect correction, gain/offset, cooling/dew and retained-frame replay. Full-frame 60- and 1,200-second captures completed in NINA. | Other revisions, illuminated-image validation, remaining SDK modes/controls, physical-disconnect retention. See [main/guide evidence](duo-capture.md) and [transfer recovery](transfer-recovery.md). |
| ASI2600MM Pro P25 USB3 (`260e`) | SDK kit: 47 exercises. Direct: 39 cases covering bins 1–4, ROI, gain/offset, ASID correction, whole-frame freshness and retained-frame retry. Same-frame processing matches SDK bytes. | Cooling/auxiliary hardware validation, cold startup, illuminated timing and physical-disconnect retention. See [ASI2600 P25 evidence](asi2600-p25.md). |
...
| ASI6200MM Pro P25 USB3 (`620b`) | Model-specific RAW16 initialization, bins 1–4, ROI, gain/offset, ASID correction, cooling/dew, fan/LED and retained-frame replay. SDK output matches independent correction byte-for-byte on the same frame. | Other revisions, illuminated-image validation, remaining SDK modes and physical-disconnect retention. See [ASI6200 evidence](asi6200-p25.md). |
| ASI6200MM Pro non-P25 USB3 (`620b`, `BC:1c=3`) | SDK extended workup and same-frame factory correction; native timing selected by hardware revision. Shares its PID with P25, which reads `BC:1c=5`. | Illuminated validation, other revisions and physical-disconnect retention. See [original ASI6200 workup](asi6200-original.md). |
| Other ASI2600 / ASI6200 Pro P25 variants | Evidence of a shared driver package/interface family. | Actual model/revision descriptors and complete SDK traces. Neither a Duo trace nor the tested mono ASI6200 establishes equivalence for other variants. |
