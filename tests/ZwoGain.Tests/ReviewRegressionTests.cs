...
public class ReviewRegressionTests
{
    ...
    static HostClient Direct() => new(Path.Combine(Root,"target/debug/zwogain-direct.exe"),"unused",simulate:true,direct:true);
    ...
    public async Task IdleReconnectionDoesNotSkipThermalSettlingForNextCapture()
    {
        ...
        using var session=new CameraSession(descriptor,()=>{
            var host=new HostClient(Path.Combine(Root,"target/debug/zwogain-host.exe"),"unused",simulate:true);
    ...
    {
        HostClient Sdk() => new(Path.Combine(Root,"target/debug/zwogain-host.exe"),Path.Combine(Root,"target/debug/selection_sdk.dll"));
        using(var host=Sdk()) {
