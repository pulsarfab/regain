...
public class CaaTests
{
    ...
    {
        string folder = Path.Combine(Path.GetTempPath(), "Regain-Caa-" + Guid.NewGuid().ToString("N"));
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), "Regain-Caa-" + Guid.NewGuid().ToString("N"), "profile.json");
        try {
