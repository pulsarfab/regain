...
$releaseJson = gh release view $Tag --repo theatrus/zwogain --json isDraft,isPrerelease,tagName,assets
...
$manifestName = "ZwoGain-$version.manifest.json"
if (@($release.assets | Where-Object name -eq $manifestName).Count -ne 1) { throw 'Release manifest is missing or ambiguous.' }
$temp = Join-Path ([IO.Path]::GetTempPath()) ('zwogain-registry-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $temp | Out-Null
gh release download $Tag --repo theatrus/zwogain --pattern $manifestName --dir $temp
...
if ($manifest.Installer.URL -cne "$baseUrl/ZwoGain-$version.zip" -or $manifest.Descriptions.FeaturedImageURL -cne "$baseUrl/zwogain.png" -or $manifest.Installer.Type -cne 'ARCHIVE' -or $manifest.Installer.ChecksumType -cne 'SHA256' -or $manifest.Installer.Checksum -notmatch '^[a-fA-F0-9]{64}$') { throw 'Unexpected release asset URLs or checksum format.' }
...
$logo = Join-Path $temp 'zwogain.png'
...
try {
    foreach ($name in @('ZwoGain.NINA.dll','ZwoGain.Core.dll','zwogain-host.exe','ASICamera2.dll','LICENSE','zwogain.png')) {
        if ($null -eq $zip.GetEntry($name)) { throw "Public archive missing $name" }
    }
    $entry = $zip.GetEntry('zwogain.png').Open()
