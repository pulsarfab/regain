...
if ($env:ZWOGAIN_CAA_TEST_CLSID) { $id = [Guid]$env:ZWOGAIN_CAA_TEST_CLSID }
...
$originChanged = $false
try {
    if ($driver.Name -ne 'ZWOgain CAA Rotator' -or $driver.InterfaceVersion -ne 3) { throw 'Wrong CAA metadata' }
    if ($driver.SupportedActions -notcontains 'ZwoGain.CAA.ResetOrigin') { throw 'Missing origin reset action' }
    if (!$Hardware) { Write-Output ('{0}-bit CAA COM metadata passed' -f ([IntPtr]::Size * 8)); return }
    $driver.Connected = $true
    $initial = $driver.Action('ZwoGain.CAA.Status','') | ConvertFrom-Json
    if ($initial.moving -or $initial.error) { throw 'CAA is not idle' }
    $driver.Sync(40)
    $originChanged = $true
    $driver.Action('ZwoGain.CAA.ResetOrigin','') | Out-Null
    ...
    $driver.Action('ZwoGain.CAA.SetReference', ('{"degrees":' + $initial.mechanical_degrees.ToString([Globalization.CultureInfo]::InvariantCulture) + '}')) | Out-Null
    $originChanged = $false
    ...
    $driver.Action('ZwoGain.CAA.SetLimit','{"degrees":361}') | Out-Null
    if (($driver.Action('ZwoGain.CAA.Status','') | ConvertFrom-Json).limit_degrees -ne 361) { throw 'Extended limit failed' }
    $driver.Action('ZwoGain.CAA.SetLimit','{"degrees":360}') | Out-Null
    ...
    try {
        if ($Hardware -and $driver.Connected) {
            $driver.Halt()
            if ($null -ne $initial) {
                if ($originChanged) { $driver.Action('ZwoGain.CAA.SetReference', ('{"degrees":' + $initial.mechanical_degrees.ToString([Globalization.CultureInfo]::InvariantCulture) + '}')) | Out-Null }
                $driver.Action('ZwoGain.CAA.SetLimit', ('{"degrees":' + $initial.limit_degrees + '}')) | Out-Null
                $driver.MoveMechanical([float]$initial.mechanical_degrees); Wait-Rotator
