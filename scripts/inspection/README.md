...
cargo run -p regain-device --locked -- zwo camera-direct --probe --cancel-read
```
...
```powershell
cargo run -p regain-device --locked -- zwo camera-direct --capture --microseconds 1000000 --frames 3
cargo run -p regain-device --locked -- zwo camera-direct --capture --width 512 --height 256 --x 16 --y 8 --gain 100 --offset 20
# Compare a second complete read with every byte of the original wire frame:
cargo run -p regain-device --locked -- zwo camera-direct --capture --replay
# Abandon 12 MiB of a replay, restart it, and compare against the original:
cargo run -p regain-device --locked -- zwo camera-direct --capture --replay-prefix-bytes 12582912
# Interrupt the first read; recover automatically without another exposure:
cargo run -p regain-device --locked -- zwo camera-direct --capture --interrupt-read-after-bytes 12582912 --replay
...
```powershell
cargo build --locked -p regain-device
target/debug/regain-device.exe zwo camera-direct --capture-duo --gain 100 --offset 50 --replay
target/debug/regain-device.exe zwo camera-direct --capture-duo --width 2080 --height 1392 --bin 3 --gain 350 --offset 120
target/debug/regain-device.exe zwo camera-direct --capture-guide --gain 100 --offset 200
...
target/debug/regain-device.exe zwo camera-direct --capture-6200 --gain 100 --offset 50 --replay
...
python scripts/inspection/validate_asi2600_p25.py --worker target/release/regain-device.exe --output artifacts/inspection/NEW-2600-p25.jsonl
