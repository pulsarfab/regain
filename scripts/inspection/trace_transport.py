...
def main():
    ...
    host = ROOT / 'target/debug/regain-device.exe'
    with args.output.open('x', encoding='utf-8') as log:
        ...
        proc = subprocess.Popen([str(host), 'zwo', 'camera-sdk', '--sdk', str(sdk)], stdin=subprocess.PIPE,
                ...
                if size:
                    ...
                    if args.compare_wire:
                        ...
                        if args.validate_direct_processing:
                            ...
                            result = subprocess.run([str(ROOT / 'target/debug/regain-device.exe'), 'zwo', 'camera-direct', '--process-frame'],
