"""ETA worker and Alpaca contract tests. Simulation only; never moves attached hardware."""
import argparse
...
def main():
    ...
    worker = helpers.Worker(binary('regain-eta'), 'SIMULATION', ['--simulate'])
    try:
    ...
    with tempfile.TemporaryDirectory(prefix='eta-alpaca-') as tmp:
        ...
        server = subprocess.Popen([binary('regain-alpaca'), '--port', str(port), '--workers', str(directory), '--profiles', str(Path(tmp)/'profiles.json'), '--no-discovery', '--simulate'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        try:
            ...
            state['profile']['serial']='SIMULATION'
